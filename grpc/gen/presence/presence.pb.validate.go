...
// ValidateAll checks the field values on MessageC with the rules defined in
// the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in MessageCMultiError, or nil
// if none found.
func (m *MessageC) ValidateAll() error {
	return m.validate(true)
}

func (m *MessageC) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	case *MessageC_A:
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, MessageCValidationError{
						field:  "A",
						reason: "embedded message failed validation",
						cause:  err,
					})
	...
	if len(errors) > 0 {
		return MessageCMultiError(errors)
	}

	return nil
}

// MessageCMultiError is an error wrapping multiple validation errors returned
// by MessageC.ValidateAll() if the designated constraints aren't met.
type MessageCMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m MessageCMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m MessageCMultiError) AllErrors() []error { return m }
